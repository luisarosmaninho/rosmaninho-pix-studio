      ...
      <article>
        <header className="px-6 md:px-12 pt-32 md:pt-48 pb-16 max-w-6xl mx-auto">
          <motion.div variants={fadeUp} initial="hidden" animate="show">
            <Link to="/diario" className="font-mono-label text-foreground/35 hover:text-copper transition-colors inline-block mb-14">
              ← Caderno de Matcha
            </Link>
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="show" transition={{ delay: 0.2 }}
          ...
          <p className="font-italic-serif text-2xl md:text-4xl leading-[1.55] text-foreground/60">{entry.excerpt}</p>
        </motion.div>
        ...
        <Fade className="px-6 md:px-12 pb-28 max-w-2xl mx-auto">
          {entry.body.slice(0, 1).map((p, i) => (
            <p key={i} className="body-text text-base md:text-lg leading-[1.9] text-foreground/78">{p}</p>
          ))}
        </Fade>
        ...
        <Fade delay={0.1} className="mb-28">
          <figure>
            ...
            <figcaption className="px-6 md:px-12 font-mono-label text-foreground/28 mt-5">{entry.photoTitle}</figcaption>
          </figure>
        </Fade>
            ...
            {olderEntry ? (
              <Link to="/diario/$slug" params={{ slug: olderEntry.slug }}
                ...
                <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}>
                  ...
                  <h3 className="font-display text-2xl md:text-3xl leading-tight group-hover:text-copper transition-colors duration-500 max-w-sm">{olderEntry.title}</h3>
            ...
            {newerEntry ? (
              <Link to="/diario/$slug" params={{ slug: newerEntry.slug }}
                ...
                <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} transition={{ delay: 0.1 }}>
                  ...
                  <h3 className="font-display text-2xl md:text-3xl leading-tight group-hover:text-copper transition-colors duration-500 max-w-sm md:ml-auto">{newerEntry.title}</h3>
